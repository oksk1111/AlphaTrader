...
def send_alert(message: str, is_error: bool = False):
    """시스템 알림 발송 - 메시지 내 [종목코드] 패턴에 종목명 자동 삽입"""
    import re

    def _inject_name(m):
        sym = m.group(1)
        name = TICKER_NAMES.get(sym, TICKER_NAMES.get(sym.upper(), ''))
        if name:
            return f"[{sym} ({name})]"
        return m.group(0)

    enriched = re.sub(r'\[([^\]]+)\]', _inject_name, message)

    prefix = "🚨 [ERROR] " if is_error else "ℹ️ [INFO] "
    full_message = f"{prefix}{enriched}\n\n⏰ {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
...
# === 종목 코드 → 종목명 매핑 (텔레그램 알림에 종목명 표시) ===
TICKER_NAMES = {
    # US 레버리지 ETF
    'TQQQ': 'ProShares UltraPro QQQ',
    'SOXL': 'Direxion Semi Bull 3X',
    'NVDL': 'GraniteShares 2x NVDA',
    'TECL': 'Direxion Tech Bull 3X',
    ...
    '091160': 'KODEX 반도체',
    # KR ETF - 위성 / 레버리지 / 기타
    '483320': 'TIGER 미국AI빅테크10',
    '483340': 'KODEX 미국AI빅테크10',
    '0041D0': 'TIGER 미국나스닥100',
    '379800': 'KODEX 미국S&P500',
    '229200': 'KODEX 코스닥150',
    '114800': 'KODEX 인버스',
    '122630': 'KODEX 레버리지',
    '233740': 'KODEX KOSDAQ150레버리지',
    '449200': 'KODEX US Tech Top10',
    '0193T0': 'KODEX SK하이닉스단일종목레버리지',
    '0193W0': 'KODEX 삼성전자단일종목레버리지',
}
