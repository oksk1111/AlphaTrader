...
def run(tickers, market, days, exchange):
    market = market.upper()
    if market == "KR":
        sets = ("v2.2_kr_stock", "v2.3_kr_stock", "v2.4_kr_stock", "v2.5_kr_stock")
    else:
        sets = ("v2.2_us", "v2.3_us", "v2.4_us", "v2.5_us")
...
def _print_verdict(report):
    ...
    for tick, info in report.get("results", {}).items():
        ...
        v25 = next((v for k, v in m.items() if k.startswith("v2.5")), None)
        ...
        if v24 and v24.get("n") and (v24.get("total_pnl_pct") or 0) > (v23.get("total_pnl_pct") or 0):
            wins_v24 += 1
        if v25 and v25.get("n") and v24 and (v25.get("total_pnl_pct") or 0) > (v24.get("total_pnl_pct") or 0):
            wins_v25 += 1
    if total == 0:
    ...
    print(f"[Verdict] v2.5 > v2.4 : {wins_v25}/{total} 종목 (Breakeven + 비용모델 효과)")
    if wins_v23 * 2 < total:
    ...
    if wins_v25 > total / 2:
        print("  ✅ v2.5 Breakeven 이 우세 → BREAKEVEN_ENABLED 유지 권장")
